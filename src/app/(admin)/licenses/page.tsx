...
export default function LicensesPage() {
  ...
  if (error) return (<div className="text-center py-20"><AlertCircle size={48} className="mx-auto mb-4" style={{ color: "#EF4444" }} /><p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>Failed to load licenses</p><Btn onClick={() => mutate()} variant="ghost"><RefreshCw size={14} />Retry</Btn></div>);
      ...
      <Modal open={showGenerate} title="Generate Device License" onClose={() => { setShowGenerate(false); setGenSuccess(false); setGenError(""); }}>
        {genSuccess ? (
          <div className="text-center py-[10px]">
            ...
            <h3 className="text-[17px] font-bold mb-2" style={{ color: "var(--text-primary)" }}>License Generated</h3>
            <p className="text-[13px] mb-4" style={{ color: "var(--text-secondary)" }}>License for <strong style={{ color: "var(--text-primary)" }}>{genDeviceId}</strong>:</p>
            <div className="flex items-center justify-between rounded-[10px] mb-5" style={{ background: "var(--bg-input)", border: "1px solid var(--border-clr)", padding: "14px 18px" }}>
              <code className="text-base font-bold" style={{ color: "#06B6D4", letterSpacing: 1 }}>{generatedKey}</code>
              <button onClick={() => navigator.clipboard?.writeText(generatedKey)} className="bg-transparent border-none cursor-pointer p-1"><Clipboard size={16} style={{ color: "var(--text-muted)" }} /></button>
            </div>
          ...
          <>
            ...
            <div className="rounded-[10px] mb-5 text-[13px] flex items-start gap-2" style={{ padding: "12px 14px", background: "rgba(6,182,212,0.15)", color: "var(--text-secondary)" }}><Info size={14} color="#06B6D4" className="shrink-0 mt-[1px]" /><span>Generate a unique license key for a registered device. The license activates immediately.</span></div>
            <InputField label="Device ID" icon={Radio} value={genDeviceId} onChange={setGenDeviceId} placeholder="Select or enter device ID" />
            <div className="mb-4"><label className="block text-xs font-semibold mb-[6px] uppercase" style={{ color: "var(--text-secondary)", letterSpacing: 0.5 }}>Registered Devices (unlicensed)</label>
              <div className="flex flex-wrap gap-[6px]">
                {unlicensedDevices.slice(0, 8).map(d => (<button key={d.id as string} onClick={() => setGenDeviceId(d.id as string)} className="rounded-lg text-xs font-medium font-sans" style={{ background: genDeviceId === d.id ? "rgba(6,182,212,0.15)" : "var(--bg-input)", border: genDeviceId === d.id ? "1px solid rgba(6,182,212,0.38)" : "1px solid var(--border-clr)", padding: "6px 12px", color: genDeviceId === d.id ? "#06B6D4" : "var(--text-secondary)", cursor: "pointer" }}>{d.id as string}</button>))}
                {unlicensedDevices.length === 0 && <span className="text-xs" style={{ color: "var(--text-muted)" }}>All devices have active licenses</span>}
              </div>
            </div>
